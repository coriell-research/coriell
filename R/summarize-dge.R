...
summarize_dge <- function(
  ...
  stopifnot(
    "Provided fdr_col column not in data.frame" = fdr_col %in% colnames(df)
  )
  stopifnot("Provided lfc_col not in data.frame" = lfc_col %in% colnames(df))

  up <- sum(
    df[[fdr_col]] < fdr & abs(df[[lfc_col]]) > lfc & df[[lfc_col]] > 0,
    na.rm = TRUE
  )
  down <- sum(
    df[[fdr_col]] < fdr & abs(df[[lfc_col]]) > lfc & df[[lfc_col]] < 0,
  ...
  data.frame(
    Direction = factor(
      c("Up", "Down", "Unperturbed"),
      levels = c("Up", "Down", "Unperturbed")
    ),
