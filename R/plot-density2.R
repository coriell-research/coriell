...
#'   )
#' 
plot_density2 <- function(x, metadata = NULL, colBy = NULL, 
                          hcl_palette = "Zissou", 
                          plot_title = NULL, x_label = "",
                          y_label = "Density", 
                          legend_position = "topright", line_weight = 1, 
                          line_type = 1, ...) {
  ...
  for (i in 1:ncol(d)) {
    line_color <- if (is.null(colBy)) "black" else cols[g[i]] 
    lines(x_values, d[, i], col = line_color, lwd = line_weight, lty = line_type)
  }
  axis(1, lwd = 0)
  axis(2, lwd = 0)
  ...
  if (!is.null(colBy)) {
    legend(legend_position, legend = levels(g), col = cols, 
           lty = line_type, lwd = line_weight, bty = "n")
  }
