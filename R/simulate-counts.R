...
#' @param n_de integer. The number of genes to be differentially expressed
...
#' @param count_offset numeric. Count to be added to the de_group
#' @return list containing the count matrix and a vector indicating which rows were modified.
#' @examples
...
simulate_counts = function(n_genes = 1000, 
  ...
  # randomly select rows of genes to be differentially expressed
  ...
  counts[de_genes, group == de_group] <- counts[de_genes, group == de_group] + count_offset
  
  rownames(counts) <- paste("gene", 1:n_genes, sep = ".")
  
  list("table" = counts, 
       "de_genes" = paste("gene", de_genes, sep = "."))
}
