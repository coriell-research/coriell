...
``` r
plot_dist(x, metadata = NULL, method = "euclidean", ...)
```
...
- x:
  ...
  numeric matrix or data.frame that can be converted to one. Samples in
...
- method:
  ...
  distance method to be used. Must be one of the distance measure to be
  used. This must be one of "euclidean", "maximum", "manhattan",
...
heatmap with distance values displayed in cells.
