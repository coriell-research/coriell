...
test_that("Check accuracy of summary results", {
  expect_equal(
    res[res$dge == "up", "n", drop = TRUE],
    25,
    label = "Up-regulated genes equal 25"
  )
  expect_equal(
    res[res$dge == "down", "n", drop = TRUE],
    25,
    label = "Down-regulated genes equal 25"
  )
  expect_equal(
    res[res$dge == "non-dge", "n", drop = TRUE],
    50,
    label = "Non-DE genes equal 50"
  )
  expect_equal(
    all(unique(res$dge) == c("up", "down", "non-dge")),
    TRUE,
    "All DE factors present"
  )
})
...
test_that("Check accuracy of summary results with different colnames", {
  expect_equal(
    res2[res2$dge == "up", "n", drop = TRUE],
    25,
    label = "Up-regulated genes equal 25"
  )
  expect_equal(
    res2[res2$dge == "down", "n", drop = TRUE],
    25,
    label = "Down-regulated genes equal 25"
  )
  expect_equal(
    res2[res2$dge == "non-dge", "n", drop = TRUE],
    50,
    label = "Non-DE genes equal 50"
  )
  expect_equal(
    all(unique(res2$dge) == c("up", "down", "non-dge")),
    TRUE,
    "All DE factors present"
  )
})
