...
We will use data from the [airwway](https://bioconductor.org/packages/3.19/data/experiment/html/airway.html) package to illustrate differential expression analysis steps. Please
...
# taking logs of zero 
offset <- log1p(y$counts) - log1p(qsd)
...
changes. This can be acheived using `glmTreat()`. **NOTE** do not use 
...
fold change must exceed this threshld before being considered for significance.
...
I have switched all of my pipelines to using competitive gene set testing in 
`edgeR` to avoid problems with correlated genes.

Below we use the `msigdb` R package to retrieve HALLMARK gene sets and then use
...
Perform gene set testing. Note here we can use `edgeR::camera()` or 
`edgeR::roast()` depending on the workflow. The above setup code provides valid 
input for either.

```{r}
# Use camera to perform competitive gene set testing
head(camera(y, idx, design, contrast = 2, inter.gene.cor = NA))
```

We can also perform a pre-ranked version of this test using `cameraPR()`. In 
...
separate sets of up and down-regulated genes and test to see of  
...
These results can be converted to data.frames with:

```{r}
head(data.frame(ego_up))
```
