...
`limma::camera()` for gene set testing.
...
`limma::mroast()`, or `limma::romer()` depending on the hypothesis being tested. 
...
We can also perform a pre-ranked version of the camera test using `cameraPR()`. In 
